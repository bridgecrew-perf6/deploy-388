...
define('www/albumdetail', function (require) {
    ...
    var show_img = function (image_id, refresh) {
        ...
        if (history.replaceState) {
            ...
            document.title = CUR_IMG.title + '_过家家家装网';
        }
    ...
    $SLIDE_PREV.click(function () {
        ...
        if (image_id) {
            ...
            if (NAV_ALBUMS.prev) {
                msg.show('已经是第一张图片，即将为你跳转到上一个相册', 'tip', 1);
                setTimeout(function () {
                    location.href = NAV_ALBUMS.prev.url + location.hash;
                }, 1000);
            } else {
                msg.show('已经是第一张图片', 'tip', 1);
            }
        }
        return false;
    });
    ...
    $SLIDE_NEXT.click(function () {
        ...
        if (image_id) {
            ...
            if (NAV_ALBUMS.next) {
                msg.show('已经是最后一张图片，即将为你跳转到下一个相册', 'tip', 1);
                setTimeout(function () {
                    location.href = NAV_ALBUMS.next.url + location.hash;
                }, 1000);
            } else {
                msg.show('已经是最后张图片', 'tip', 1);
            }
        }
        return false;
    });
    ...
    show_image_thumb(function (scale_style) {
        $('.g-slide-img img').css(scale_style);
    });
    scroll_center();
    update_slide_nav();
